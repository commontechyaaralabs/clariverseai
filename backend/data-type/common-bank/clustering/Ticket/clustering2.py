...
print(f"tickets Collection: {tickets_COLLECTION_NAME}")
...
for idx, (doc_data, cluster_id) in enumerate(zip(document_data, labels)):
    ...
    cluster_data[adjusted_cluster_id]['tickets_ids'].append(str(doc_data["_id"]))
...
def merge_cluster_data(existing_doc, new_data):
    ...
    existing_ticket_ids = set(existing_doc.get('tickets_ids', []))
    new_ticket_ids = set(new_data['tickets_ids'])
    ...
    if check_cluster_exists(cluster_id):
        ...
        cluster_update = {
            ...
            "update": {"$set": {
                ...
                "tickets_ids": merged_data['tickets_ids'],
...
# Prepare tickets collection updates
for idx, (doc_data, original_cluster_id) in enumerate(zip(document_data, labels)):
    ...
    tickets_updates.append(tickets_update)
...
print("Updating tickets collection with K-means cluster information...")
tickets_updates_successful = 0
tickets_updates_failed = 0

for update_data in tickets_updates:
    try:
        ...
        if result.modified_count > 0 or result.matched_count > 0:
            tickets_updates_successful += 1
        else:
            tickets_updates_failed += 1
            
    except Exception as e:
        tickets_updates_failed += 1
        print(f"Error updating tickets document {update_data['filter']['_id']}: {e}")

print(f"Successfully updated {tickets_updates_successful} tickets documents with K-means cluster information")
if tickets_updates_failed > 0:
    print(f"Failed to update {tickets_updates_failed} tickets documents")

# Verify updates in both collections
tickets_verification_count = tickets_collection.count_documents({"kmeans_cluster_id": {"$exists": True}})
cluster_verification_count = cluster_collection.count_documents({})

print(f"Verification - tickets collection: {tickets_verification_count} documents now have kmeans_cluster_id field")
...
print(f"tickets documents updated: {tickets_updates_successful}")
print("\nCollections updated:")
print(f"1. {tickets_COLLECTION_NAME} - Added/Updated fields:")
...
print("   - tickets_ids: Array of tickets document IDs in cluster")
print("   - data: 'tickets'")
