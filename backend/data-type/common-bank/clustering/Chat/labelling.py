...
class UniqueBankingClusterLabeler:
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            # Create indexes on chat-chunks collection
            self.db["chat-chunks"].create_index("kmeans_cluster_keyphrase.cluster_id")
            self.db["chat-chunks"].create_index("kmeans_cluster_keyphrase.label")
    ...
    async def update_chat_chunks_with_labels(self):
        """Update chat-chunks collection with dominant_label and subcluster labels"""
        try:
            logger.info("Starting to update chat-chunks collection with labels...")
            ...
            # Process chat-chunks in batches
            batch_size = 1000
            total_messages = self.db["chat-chunks"].count_documents({})
            ...
            for skip in range(0, total_messages, batch_size):
                try:
                    ...
                    email_messages = list(self.db["chat-chunks"].find(
                        {},
                        {
                            "_id": 1,
                    ...
                    if bulk_operations:
                        try:
                            result = self.db["chat-chunks"].bulk_write(bulk_operations, ordered=False)
            ...
            logger.info(f"Completed updating chat-chunks collection. Updated: {updated_count}, Errors: {error_count}")
            ...
            logger.error(f"Error updating chat-chunks collection: {e}")
    ...
    async def process_all_clusters(self):
        ...
        try:
            ...
            # Update chat-chunks collection with labels
            logger.info("Updating chat-chunks collection with labels...")
            email_update_result = await self.update_chat_chunks_with_labels()
    ...
    async def get_cluster_summary(self):
        ...
        try:
            ...
            # Check chat-chunks collection statistics
            total_emails = self.db["chat-chunks"].count_documents({})
            emails_with_dominant_labels = self.db["chat-chunks"].count_documents({
                "kmeans_cluster_keyphrase.dominant_label": {"$exists": True}
            })
            emails_with_subcluster_labels = self.db["chat-chunks"].count_documents({
    ...
    async def regenerate_duplicate_labels(self):
        ...
        try:
            ...
            # If labels were regenerated, update chat-chunks collection
            if regenerated_count > 0:
                logger.info("Updating chat-chunks collection after label regeneration...")
                email_update_result = await self.update_chat_chunks_with_labels()
    ...
    async def validate_chat_chunks_mapping(self):
        """Validate the mapping of labels in chat-chunks collection"""
        try:
            logger.info("Validating chat-chunks label mapping...")

            # Get sample of chat-chunks with labels
            sample_emails = list(self.db["chat-chunks"].find(
                {
            ...
            logger.error(f"Error validating chat-chunks mapping: {e}")
            return {"status": "error", "message": str(e)}
...
async def main():
    ...
    try:
        ...
        # Validate chat-chunks mapping
        logger.info("Validating chat-chunks label mapping...")
        mapping_validation = await labeler.validate_chat_chunks_mapping()
        logger.info(f"chat-chunks mapping validation: {mapping_validation}")
...
async def export_chat_chunks_with_labels(db: Database, output_file="chat_chunks_with_labels_export.json"):
    """Export chat chunks with their mapped labels"""
    try:
        # Get sample of chat chunks with labels
        chat_chunks_with_labels = list(db["chat-chunks"].find(
            {
                "kmeans_cluster_keyphrase.dominant_label": {"$exists": True}
            },
            {
        ...
        logger.error(f"Error exporting chat-chunks with labels: {e}")
        return {"status": "error", "message": str(e)}
...
if __name__ == "__main__":
    ...
    try:
        ...
        if mongo_connection_string and mongo_database_name:
            ...
            # Export chat-chunks with labels
            chat_chunks_export_result = asyncio.run(export_chat_chunks_with_labels(db))
            print(f"Chat chunks export result: {chat_chunks_export_result}")
