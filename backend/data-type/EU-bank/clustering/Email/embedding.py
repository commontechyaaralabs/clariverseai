...
print(f"Found {len(documents)} documents with dominant_topic in enauknessages collection")
...
print("\nPerforming comprehensive final verification on enauknessages collection...")

# Check total documents with embeddings
final_embedding_count = collection.count_documents({"embeddings": {"$exists": True}})
print(f"Total documents in enauknessages collection with 'embeddings' field: {final_embedding_count}")
...
print(f"Documents updated in enauknessages collection: {final_embedding_count}")
...
print(f"\nEmbeddings saved in 'embeddings' field in 'enauknessages' collection")
print("="*60)