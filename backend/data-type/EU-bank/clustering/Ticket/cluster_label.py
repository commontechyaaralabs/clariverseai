...
class UniqueBankingClusterLabeler:
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            # Create indexes on tickets collection
            self.db["tickets"].create_index("kmeans_cluster_keyphrase.cluster_id")
            self.db["tickets"].create_index("kmeans_cluster_keyphrase.label")
    ...
    async def update_tickets_with_labels(self):
        """Update tickets collection with dominant_label and subcluster labels"""
        try:
            logger.info("Starting to update tickets collection with labels...")
            ...
            batch_size = 1000
            total_messages = self.db["tickets"].count_documents({})
            ...
            for skip in range(0, total_messages, batch_size):
                try:
                    ...
                    email_messages = list(self.db["tickets"].find(
                        {},
                        {
                            "_id": 1,
                    ...
                    if bulk_operations:
                        try:
                            result = self.db["tickets"].bulk_write(bulk_operations, ordered=False)
            ...
            logger.info(f"Completed updating tickets collection. Updated: {updated_count}, Errors: {error_count}")
            ...
            logger.error(f"Error updating tickets collection: {e}")
    ...
    async def process_all_clusters(self):
        ...
        try:
            ...
            # Update tickets collection with labels
            logger.info("Updating tickets collection with labels...")
            email_update_result = await self.update_tickets_with_labels()
    ...
    async def get_cluster_summary(self):
        ...
        try:
            ...
            # Check tickets collection statistics
            total_emails = self.db["tickets"].count_documents({})
            emails_with_dominant_labels = self.db["tickets"].count_documents({
                "kmeans_cluster_keyphrase.dominant_label": {"$exists": True}
            })
            emails_with_subcluster_labels = self.db["tickets"].count_documents({
    ...
    async def regenerate_duplicate_labels(self):
        ...
        try:
            ...
            # If labels were regenerated, update tickets collection
            if regenerated_count > 0:
                logger.info("Updating tickets collection after label regeneration...")
                email_update_result = await self.update_tickets_with_labels()
    ...
    async def validate_tickets_mapping(self):
        """Validate the mapping of labels in tickets collection"""
        try:
            logger.info("Validating tickets label mapping...")
            
            # Get sample of tickets with labels
            sample_emails = list(self.db["tickets"].find(
                {
            ...
            logger.error(f"Error validating tickets mapping: {e}")
            return {"status": "error", "message": str(e)}
...
async def main():
    ...
    try:
        ...
        # Validate tickets mapping
        logger.info("Validating tickets label mapping...")
        mapping_validation = await labeler.validate_tickets_mapping()
        logger.info(f"tickets mapping validation: {mapping_validation}")
...
async def export_tickets_with_labels(db: Database, output_file="tickets_with_labels_export.json"):
    """Export tickets with their mapped labels"""
    try:
        # Get sample of tickets with labels
        emails_with_labels = list(db["tickets"].find(
            {
                "kmeans_cluster_keyphrase.dominant_label": {"$exists": True}
            },
            {
        ...
        logger.error(f"Error exporting tickets with labels: {e}")
        return {"status": "error", "message": str(e)}
...
if __name__ == "__main__":
    ...
    try:
        ...
        if mongo_connection_string and mongo_database_name:
            ...
            # Export tickets with labels
            email_export_result = asyncio.run(export_tickets_with_labels(db))
