...
def remove_existing_sentiment(collection):
    """Remove existing sentiment field from all records"""
    print("\n🔄 Step 1: Removing existing sentiment fields...")
    
    # Check how many records have the sentiment field
    records_with_sentiment = collection.count_documents({"sentiment": {"$exists": True}})
    
    if records_with_sentiment == 0:
        print("✅ No existing sentiment fields found")
        return
    
    print(f"📊 Found {records_with_sentiment} records with existing sentiment field")
    
    # Remove sentiment field from all records
    try:
        ...
        print(f"✅ Successfully removed sentiment field from {result.modified_count} records")
        
        # Verify removal
        remaining_sentiment = collection.count_documents({"sentiment": {"$exists": True}})
        if remaining_sentiment == 0:
            print("✅ All existing sentiment fields have been successfully removed!")
        else:
            print(f"⚠️ {remaining_sentiment} records still have sentiment field")
            
    except Exception as e:
        print(f"❌ Error removing sentiment field: {e}")

def update_sentiment_based_on_priority(collection):
    """Update sentiment based on priority field"""
    print("\n🔄 Step 2: Adding sentiment based on priority...")
        ...
        for record in batch_records:
            try:
                ...
                if priority in PRIORITY_SENTIMENT_MAP:
                    ...
                    # Update the record with new sentiment
...
def get_sentiment_statistics(collection):
    ...
    records_with_sentiment = collection.count_documents({"sentiment": {"$exists": True, "$ne": None}})
    ...
    if records_without_sentiment > 0:
        print(f"\n⚠️ Records without sentiment: {records_without_sentiment}")
...
def main():
    """Main function"""
    print("🚀 Starting Sentiment Update Process for Tickets Collection")
    print("=" * 60)
    print("Priority to Sentiment Mapping:")
    for priority, sentiment in PRIORITY_SENTIMENT_MAP.items():
        print(f"  {priority} → sentiment: {sentiment}")
    print("=" * 60)
    ...
    try:
        ...
        # Step 1: Remove existing sentiment fields
        remove_existing_sentiment(collection)
        
        # Step 2: Add new sentiment based on priority
