# EU Banking Trouble Ticket Content Generator - OpenRouter Migration
import os
import random
import time
import json
...
MAIN_LOG_FILE = LOG_DIR / f"ticket_generator_openrouter_{timestamp}.log"
...
class CheckpointManager:
    ...
    async def increment_retry(self, ticket_id):
        async with self._lock:
...
async def generate_ticket_content(ticket_data):
    """Generate ticket content with OpenRouter"""
    ...
    for attempt in range(MAX_RETRY_ATTEMPTS_PER_TICKET):
        ...
        try:
            if attempt > 0:
                ...
                await checkpoint_manager.increment_retry(ticket_id)
            
            result = await _process_single_ticket_internal(ticket_record, total_tickets)
            ...
            logger.warning(f"Ticket {ticket_id}: Timeout on attempt {attempt + 1}/{MAX_RETRY_ATTEMPTS_PER_TICKET}")
            if attempt < MAX_RETRY_ATTEMPTS_PER_TICKET - 1:
                continue
            else:
                logger.error(f"Ticket {ticket_id}: Failed after {MAX_RETRY_ATTEMPTS_PER_TICKET} timeout attempts")
...
async def process_tickets_optimized():
    ...
    try:
        ...
        ticket_records = ticket_col.find(query).batch_size(100)
    ...
    try:
        ...
        while processed_count < total_tickets:
            if shutdown_flag.is_set():
            ...
            logger.info(f"Processing batch {batch_num}/{total_batches} (tickets {processed_count-len(batch)+1}-{processed_count})")
            ...
            if processed_count < total_tickets and not shutdown_flag.is_set():
