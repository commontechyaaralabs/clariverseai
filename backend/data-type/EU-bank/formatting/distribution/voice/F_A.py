...
def add_follow_up_and_action_fields():
    """
    Add follow_up_required and action_required_status fields to voice calls
    based on urgency and category distribution
    """
    
    print("=" * 80)
    print("ADDING FOLLOW-UP AND ACTION REQUIRED FIELDS TO VOICE CALLS")
    print("=" * 80)
    ...
    mismatch = voice_collection.count_documents({
        'follow_up_required': 'yes',
    ...
    follow_up_no_count = voice_collection.count_documents({'follow_up_required': 'no'})
    action_no_count = voice_collection.count_documents({
        'follow_up_required': 'no',
        'action_required_status': 'no'
    })
    ...
    csv_filename = f"voice_follow_up_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
