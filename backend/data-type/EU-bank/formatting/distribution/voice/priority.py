...
def rename_action_required_to_pending():
    ...
    sample_doc = voice_collection.find_one({'action_required_status': {'$exists': True}})
    if not sample_doc:
    ...
    total_docs = voice_collection.count_documents({'action_required_status': {'$exists': True}})
    ...
    # Verify the rename
    renamed_count = voice_collection.count_documents({'action_pending_status': {'$exists': True}})
    old_count = voice_collection.count_documents({'action_required_status': {'$exists': True}})
...
def update_priority_distribution():
    """
    Update priority field in voice calls based on urgency, follow_up_required, 
    and action_pending_status distribution
    """
    
    print("=" * 80)
    print("UPDATING PRIORITY DISTRIBUTION IN VOICE CALLS")
    print("=" * 80)
    ...
    sample_doc = voice_collection.find_one()
    if sample_doc:
    ...
    actual_combinations = list(voice_collection.aggregate(pipeline))
    ...
    for (urgency, follow_up, action), priority_list in combinations.items():
        ...
        docs = list(voice_collection.find(query))
        ...
        for follow_up in ['yes', 'no']:
            for action in ['yes', 'no']:
                for priority in ['P1-Critical', 'P2-High', 'P3-Medium', 'P4-Low', 'P5-Very Low']:
                    count = voice_collection.count_documents({
                        'urgency': urgency,
                        'follow_up_required': follow_up,
                        'action_pending_status': action,
                        'priority': priority
    ...
    csv_filename = f"voice_priority_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
