...
def process_all_records_from_db():
    """
    Process all records from MongoDB with realistic date distribution
    """
    # Date range
    start_date = datetime(2025, 1, 1, 0, 0, 0)
    end_date = datetime(2025, 6, 30, 23, 59, 59)
    
    print(f"Reading from MongoDB collection: {email_collection.name}")
    
    # Get all records from database
    records = list(email_collection.find({}))
    ...
    print(f"\nUpdating {len(fixed_records)} records in database...")
    for i, fixed_record in enumerate(fixed_records, 1):
        try:
            email_collection.update_one(
                {"_id": fixed_record["_id"]},
                {"$set": fixed_record}
            )
            if i % 200 == 0:
                print(f"  Updated {i}/{len(fixed_records)} records...")
        except Exception as e:
