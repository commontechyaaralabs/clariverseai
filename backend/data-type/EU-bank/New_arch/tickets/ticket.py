...
# OpenRouter setup
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY", "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")
OPENROUTER_BASE_URL = "https://openrouter.ai/api/v1"
OPENROUTER_URL = f"{OPENROUTER_BASE_URL}/chat/completions"
OPENROUTER_MODEL = "google/gemma-3-27b-it:free"

# Optimized batch processing configuration
BATCH_SIZE = 3  # Process 3 records per batch as requested
CPU_COUNT = multiprocessing.cpu_count()
MAX_WORKERS = min(CPU_COUNT * 2, 16)  # Reduced for stability with batch size of 3
REQUEST_TIMEOUT = 120
MAX_RETRIES = 3
RETRY_DELAY = 1
...
def call_openrouter_with_backoff(prompt, timeout=REQUEST_TIMEOUT):
    ...
    headers = {
        ...
        'X-Title': 'EU Banking Ticket Generator'  # Optional but recommended
    }
    ...
    try:
        ...
        if "message" not in result["choices"][0] or "content" not in result["choices"][0]["message"]:
            logger.error(f"Invalid message structure in response")
            raise KeyError("Invalid message structure in OpenRouter response")
            
        # Add delay to help with rate limiting (increased for free tier)
        time.sleep(5)  # 5 second delay for free tier rate limits
...
def update_tickets_with_content_parallel():
    ...
    logger.info("Waiting 10 seconds before testing OpenRouter connection to avoid rate limits...")
    time.sleep(10)
    if not test_openrouter_connection():
        ...
        logger.error("For free tier, consider waiting 5-10 minutes between runs.")
        return
    ...
    try:
        for batch_num in range(1, total_batches + 1):
            ...
            # Brief pause between batches (increased for rate limiting)
            if not shutdown_flag.is_set() and batch_num < total_batches:
                time.sleep(10)  # 10 second delay between batches for rate limiting
...
def test_openrouter_connection():
    """Test if OpenRouter is accessible and API key is valid"""
    try:
        logger.info(f"Testing connection to OpenRouter: {OPENROUTER_BASE_URL}")
        ...
        headers = {
            ...
            'X-Title': 'EU Banking Ticket Generator Test'
        }
        ...
        test_payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "Generate a simple JSON object with 'test': 'success'"
                }
            ],
            "max_tokens": 50,
            "temperature": 0.1
        ...
        try:
            ...
            if "choices" in result and result["choices"] and "message" in result["choices"][0]:
                ...
                if "usage" in result:
                    logger.info(f"Usage - Prompt tokens: {result['usage'].get('prompt_tokens', 'N/A')}, Completion tokens: {result['usage'].get('completion_tokens', 'N/A')}")
                
                return True
            else:
                logger.error(f"Invalid response structure. Fields: {list(result.keys())}")
                return False
        ...
        logger.error("Connection test timed out - OpenRouter endpoint may be unreachable")
        return False
    except requests.exceptions.ConnectionError:
        logger.error("Connection error - OpenRouter endpoint is not reachable")
        return False
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 401:
            logger.error("Authentication failed - Invalid OpenRouter API key")
...
def main():
    ...
    logger.info(f"OpenRouter URL: {OPENROUTER_BASE_URL}")
