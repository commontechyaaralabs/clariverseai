# EU Banking Email - Assign Stages to Records Without Stages Field
import os
import logging
...
def assign_stages_to_records():
    ...
    # Initialize database
    client, email_col = init_database()
    ...
    try:
        ...
        records_without_stages = list(email_col.find(
            {"stages": {"$exists": False}},
            {"_id": 1}
        ))
        ...
        for stage in stages:
            count = email_col.count_documents({"stages": stage})
            logger.info(f"  {stage}: {count} records")
        
        # Verify no records are left without stages
        remaining_without_stages = email_col.count_documents({"stages": {"$exists": False}})
