...
type Email = DocumentResponse;

const EmailTopicAnalysis = () => {
  ...
  const [pageCache, setPageCache] = useState<Map<number, { documents: Email[], pagination: any }>>(new Map());
  ...
  useEffect(() => {
    const loadClusterOptions = async () => {
      try {
        setLoading(true);
        const response = await fetchClusterOptions('email', 'banking');
  ...
  useEffect(() => {
    const loadDocuments = async () => {
      ...
      if (pageCache.has(currentPage)) {
        ...
        setEmails(cachedData.documents);
      ...
      try {
        ...
        if (response.status === 'success') {
          ...
          console.log(`Cached page ${currentPage} with ${response.documents.length} emails`);
          
          setEmails(response.documents);
  ...
  const getFilteredEmails = () => {
    ...
    return (
        email.message_id?.toLowerCase().includes(searchLower) ||
        email.sender_id?.toLowerCase().includes(searchLower) ||
        email.subject?.toLowerCase().includes(searchLower) ||
        email.message_text?.toLowerCase().includes(searchLower) ||
  ...
  // Use the emails directly from the API response since they're already paginated
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="flex-1 flex overflow-hidden">
                  ...
                  <div className="w-full bg-gray-800 bg-opacity-50 overflow-y-auto">
                    ...
                    ) : filteredEmails.length === 0 && !loadingDocuments ? (
                      <div className="p-4 text-center text-gray-300">
                            ...
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex-1">
                                  <div className="font-medium text-white text-base mb-1">{email.subject || 'No Subject'}</div>
                                  <div className="text-sm text-gray-300">{email.sender_name || email.sender_id || 'Unknown Sender'}</div>
                                </div>
                            ...
                            {selectedEmail?._id === email._id && (
                              <div data-email-details className="w-2/3 bg-gray-800 bg-opacity-80 border-l border-gray-700 p-6">
                                ...
                                <div className="border-b border-gray-600 pb-4 mb-4">
                                  <div className="text-sm text-gray-300">From: {selectedEmail.sender_name || selectedEmail.sender_id || 'N/A'}</div>
                                  <div className="text-sm text-gray-300">To: {selectedEmail.receiver_names?.join(', ') || selectedEmail.receiver_ids?.join(', ') || 'N/A'}</div>
                                  <div className="text-sm text-gray-300">Timestamp: {selectedEmail.timestamp || 'N/A'}</div>
                                </div>
...
export default EmailTopicAnalysis; 