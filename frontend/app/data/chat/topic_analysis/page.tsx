...
type ChatMessage = DocumentResponse;

const ChatTopicAnalysis = () => {
  ...
  const [pageCache, setPageCache] = useState<Map<number, { documents: ChatMessage[], pagination: any }>>(new Map());
  ...
  useEffect(() => {
    const loadClusterOptions = async () => {
      try {
        setLoading(true);
        const response = await fetchClusterOptions('chat', 'banking');
  ...
  useEffect(() => {
    const loadDocuments = async () => {
      ...
      if (pageCache.has(currentPage)) {
        ...
        setMessages(cachedData.documents);
      ...
      try {
        ...
        if (response.status === 'success') {
          ...
          setPageCache(prev => new Map(prev).set(currentPage, { documents: response.documents, pagination: response.pagination }));
          console.log(`Cached page ${currentPage} with ${response.documents.length} messages`);
  ...
  const getFilteredMessages = () => {
    ...
    return (
        message.chat_id?.toLowerCase().includes(searchLower) ||
        message.dominant_cluster_label?.toLowerCase().includes(searchLower) ||
        message.subcluster_label?.toLowerCase().includes(searchLower) ||
        message.cleaned_text?.toLowerCase().includes(searchLower)
      );
    });
  };
  ...
  const handleMessageClick = (message: ChatMessage) => {
    // If clicking the same message that's already selected, close the details
  ...
  // Use the messages directly from the API response since they're already paginated
          ...
          <div className="max-w-[95vw] mx-auto">
            <div className="text-center mb-4">
              ...
              <p className="text-xl text-gray-300 max-w-4xl mx-auto mb-8">
                Explore and filter chat messages by topics and subtopics to gain actionable insights
              </p>
            </div>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="flex-1 flex overflow-hidden">
                  ...
                  <div className="w-full bg-gray-800 bg-opacity-50 overflow-y-auto">
                    ...
                    ) : filteredMessages.length === 0 && !loadingDocuments ? (
                      <div className="p-4 text-center text-gray-300">
                          ...
                          <div key={message._id} className="flex">
                            ...
                            <div
                              ...
                              className={`p-4 border-b border-gray-700 hover:bg-gray-700 cursor-pointer flex-1 ${
                                selectedMessage?._id === message._id ? 'bg-gray-700' : ''
                              }`}
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex-1">
                                  <div className="font-medium text-white text-base">
                                    {message.chat_members?.slice(0, 2).map((member: unknown) => (member as { display_name?: string; id?: string })?.display_name || (member as { display_name?: string; id?: string })?.id || 'Unknown').join(', ') || 'No Members'}
                                  </div>
                                </div>
                            ...
                            {/* Message Details - appears right next to selected item */}
                            {selectedMessage?._id === message._id && (
                              <div data-chat-details className="w-2/3 bg-gray-800 bg-opacity-80 border-l border-gray-700 p-6">
                                <div className="flex justify-between items-center mb-4">
                                  <h2 className="text-xl font-semibold text-white">Chat ID: {selectedMessage.chat_id}</h2>
                                  <button
                                ...
                                <div className="border-b border-gray-600 pb-6 mb-6">
                                  <div className="text-base text-gray-300 mb-2">Total Messages: {selectedMessage.total_messages || 'N/A'}</div>
                                  <div className="text-base text-gray-300">Members: {selectedMessage.chat_members?.map((member: unknown) => (member as { display_name?: string; id?: string })?.display_name || (member as { display_name?: string; id?: string })?.id || 'Unknown').join(', ') || 'N/A'}</div>
                                </div>
                                <div className="space-y-6">
                                  <div>
                                    ...
                                    <div className="mt-1 p-6 bg-gray-900 rounded-lg text-gray-200 max-h-[600px] overflow-y-auto">
                                      {selectedMessage.raw_segments && selectedMessage.raw_segments.length > 0 ? (
                                        <div className="space-y-2">
                                          {selectedMessage.raw_segments.map((segment: any, index: number) => {
                                            ...
                                            // Use the first sender as "other" and alternate from there
                                            const firstSegment = selectedMessage.raw_segments?.[0] as any;
                                            const firstSender = firstSegment?.sender_name || firstSegment?.sender || firstSegment?.speaker || 'Unknown';
                                  ...
                                  <div className="grid grid-cols-2 gap-6">
                                    <div>
                                      ...
                                      <div className="mt-1 text-pink-400 font-medium text-lg">{selectedMessage.dominant_cluster_label || 'N/A'}</div>
                                    </div>
                                    <div>
                                      <label className="text-base font-medium text-gray-300 mb-2">Subcluster Label</label>
                                      <div className="mt-1 text-purple-400 font-medium text-lg">{selectedMessage.subcluster_label || 'N/A'}</div>
                                    </div>
                                  </div>
                                  <div className="grid grid-cols-2 gap-6">
                                    <div>
                                      <label className="text-base font-medium text-gray-300 mb-2">Dominant Topic</label>
                                      <div className="mt-1 text-gray-200 text-lg">{selectedMessage.dominant_topic || 'N/A'}</div>
                                    </div>
                                    <div>
                                      <label className="text-base font-medium text-gray-300 mb-2">Subtopics</label>
                                      <div className="mt-1 text-gray-200 text-lg">{selectedMessage.subtopics || 'N/A'}</div>
                                    </div>
                                  </div>
                                </div>
                              </div>
...
export default ChatTopicAnalysis; 