...
const VoiceTopicAnalysis = () => {
  ...
  const [selectedVoiceMessage, setSelectedVoiceMessage] = useState<VoiceMessage | null>(null);
  ...
  const [pageCache, setPageCache] = useState<Map<number, { documents: VoiceMessage[], pagination: any }>>(new Map());
  ...
  useEffect(() => {
    const loadClusterOptions = async () => {
      try {
        setLoading(true);
        const response = await fetchClusterOptions('voice', 'banking');
  ...
  useEffect(() => {
    ...
    const loadDocuments = async () => {
      ...
      if (pageCache.has(currentPage)) {
        ...
        console.log(`Cached data for page ${currentPage}:`, cachedData.documents.length, 'voice messages');
        setVoiceMessages(cachedData.documents);
      ...
      try {
        ...
        if (response.status === 'success') {
          ...
          setPageCache(prev => {
            ...
            console.log(`Cached page ${currentPage} with ${response.documents.length} voice messages`);
            console.log(`Cache size after caching: ${newCache.size}`);
            console.log(`Cache keys after caching:`, Array.from(newCache.keys()));
            return newCache;
          });
          
          setVoiceMessages(response.documents);
  ...
  const getFilteredVoiceMessages = () => {
    if (!searchTerm) return voiceMessages;
    
    return voiceMessages.filter(voiceMessage => {
      const searchLower = searchTerm.toLowerCase();
    return (
        voiceMessage.call_id?.toLowerCase().includes(searchLower) ||
        voiceMessage.customer_name?.toLowerCase().includes(searchLower) ||
        voiceMessage.call_purpose?.toLowerCase().includes(searchLower) ||
        voiceMessage.dominant_cluster_label?.toLowerCase().includes(searchLower) ||
        voiceMessage.subcluster_label?.toLowerCase().includes(searchLower)
      );
    });
  };
  ...
  const filteredVoiceMessages = getFilteredVoiceMessages();
  // Use the voice messages directly from the API response since they're already paginated
  const paginatedVoiceMessages = filteredVoiceMessages;
          ...
          <div className="max-w-[95vw] mx-auto">
            <div className="text-center mb-4">
              ...
              <p className="text-xl text-gray-300 max-w-4xl mx-auto mb-8">
                Explore and filter voice messages by topics and subtopics to gain actionable insights
              </p>
            </div>
              ...
              <div className="flex-1 flex flex-col">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="relative min-w-[300px]">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-3 bg-gray-700 text-gray-200 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm"
                      />
                    </div>
                  </div>
                  <div className="text-sm text-gray-300">
                    Showing {pagination.page_document_count} of {pagination.total_documents} messages
                ...
                <div className="flex-1 flex overflow-hidden">
                  ...
                  <div className="w-full bg-gray-800 bg-opacity-50 overflow-y-auto">
                    ...
                    ) : filteredVoiceMessages.length === 0 && !loadingDocuments ? (
                      <div className="p-4 text-center text-gray-300">
                        No messages match the selected criteria. Try adjusting your filters.
                      </div>
                    ) : (
                      <>
                        {/* Column Header */}
                        ...
                        {paginatedVoiceMessages.map((voiceMessage) => (
                          <div key={voiceMessage._id} className="flex">
                            ...
                            {selectedVoiceMessage?._id === voiceMessage._id && (
                              <div data-voice-details className="w-2/3 bg-gray-800 bg-opacity-80 border-l border-gray-700 p-6">
                                <div className="flex justify-between items-center mb-4">
                                  <h2 className="text-xl font-semibold text-white">{selectedVoiceMessage.customer_name || selectedVoiceMessage.call_id || 'No Customer Name'}</h2>
                                  <button
                                ...
                                <div className="border-b border-gray-600 pb-6 mb-6">
                                  <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                      <div className="text-sm text-gray-300">Call ID: {selectedVoiceMessage.call_id || 'N/A'}</div>
                                      <div className="text-sm text-gray-300">Customer: {selectedVoiceMessage.customer_name || 'N/A'}</div>
                                    </div>
                                    <div className="space-y-2">
                                      <div className="text-sm text-gray-300">Customer ID: {selectedVoiceMessage.customer_id || 'N/A'}</div>
                                      <div className="text-sm text-gray-300">Email: {selectedVoiceMessage.email || 'N/A'}</div>
                                    </div>
                                  </div>
                                </div>
                                <div className="space-y-6">
                                  <div>
                                    ...
                                    <div className="mt-1 p-6 bg-gray-900 rounded-lg text-gray-200 max-h-[600px] overflow-y-auto">
                                      {selectedVoiceMessage.conversation && selectedVoiceMessage.conversation.length > 0 ? (
                                        <div className="space-y-2">
                                          {selectedVoiceMessage.conversation.map((msg: any, index: number) => {
                                  ...
                                  <div className="grid grid-cols-2 gap-4">
                                    <div>
                                      ...
                                      <div className="mt-1 text-pink-400 font-medium">{selectedVoiceMessage.dominant_cluster_label || 'N/A'}</div>
                                    </div>
                                    <div>
                                      <label className="text-sm font-medium text-gray-300">Subcluster Label</label>
                                      <div className="mt-1 text-purple-400 font-medium">{selectedVoiceMessage.subcluster_label || 'N/A'}</div>
                                    </div>
                                  </div>
                                  <div className="grid grid-cols-2 gap-4">
                                    <div>
                                      <label className="text-sm font-medium text-gray-300">Dominant Topic</label>
                                      <div className="mt-1 text-gray-200">{selectedVoiceMessage.dominant_topic || 'N/A'}</div>
                                    </div>
                                    <div>
                                      <label className="text-sm font-medium text-gray-300">Subtopics</label>
                                      <div className="mt-1 text-gray-200">{selectedVoiceMessage.subtopics || 'N/A'}</div>
                                    </div>
                                  </div>
                                  <div>
                                    ...
                                    <div className="mt-1">
                                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                                        selectedVoiceMessage.sentiment === 'Positive' ? 'bg-green-600 text-white' :
                                        selectedVoiceMessage.sentiment === 'Negative' ? 'bg-red-600 text-white' :
                                        selectedVoiceMessage.sentiment === 'Neutral' ? 'bg-gray-600 text-white' :
                                        'bg-gray-500 text-white'
                                      }`}>
                                        {selectedVoiceMessage.sentiment || 'N/A'}
                                      </span>
                                    </div>
                                  </div>
                                </div>
                        ...
                        {pagination.total_pages > 1 && (
                          <div className="p-4 flex justify-between items-center bg-gray-800 bg-opacity-80 border-t border-gray-700">
                            ...
                            <div className="text-sm text-gray-300">
                              Page {pagination.current_page} of {pagination.total_pages} • Showing {pagination.page_document_count} of {pagination.total_documents} messages
                            </div>
                            <button
...
export default VoiceTopicAnalysis;
